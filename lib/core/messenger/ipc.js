...
class Messenger extends EventEmitter {
   ...
   * @return {Messenger} this
   */
  // 底层利用sendmessage模块来跨进程通信
  sendTo(pid, action, data) {
    debug('[%s] send %s with %j to %s', this.pid, action, data, pid);
    // 其底层是调用subprocess.send来进行通信的 :https://nodejs.org/dist/latest-v10.x/docs/api/child_process.html#child_process_subprocess_send_message_sendhandle_options_callback
    sendmessage(process, {
      action,
      data,
